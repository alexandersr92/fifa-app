// Public page to join a tournament. Displays the tournament details and the
// list of registered players. Guests can join by providing a display name.
...
export default function JoinTournamentPage() {
  ...
  return (
    <div className="max-w-sm mx-auto p-4 py-8 space-y-4">
      ...
      {sessionInfo ? (
        <>
          <h1 className="text-3xl font-bold mb-1">{sessionInfo.title}</h1>
          {sessionInfo.description && <p className="text-text-muted mb-3">{sessionInfo.description}</p>}
          <p className="text-text-muted mb-3">Jugadores: {sessionInfo.players_count}/{sessionInfo.max_players}</p>
          ...
          {!joined ? (
            ...
            <p className="text-green-500">¡Inscripción exitosa! Espera a que el anfitrión inicie el torneo.</p>
          )}
        </>
      ) : (
